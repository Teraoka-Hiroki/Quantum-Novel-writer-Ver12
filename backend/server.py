...
@app.route('/api/update_adoption', methods=['POST'])
def update_adoption():
    global DATA_STORE
    req = request.json
    item_id = req.get('id')
...
@app.route('/api/custom_optimize', methods=['POST'])
def custom_optimize():
    global DATA_STORE
    req = request.json
    if req.get('amplify_token'): DATA_STORE['amplify_token'] = req.get('amplify_token')
    ...
    try:
        result = LogicHandler.run_custom_optimization(
            DATA_STORE['amplify_token'],
            DATA_STORE['candidates'],
            DATA_STORE['params']
        )
...
def generate_draft():
    ...
    if not selected:
        return jsonify({"status": "error", "message": "最適化された要素がありません。タブ2で最適化を実行してください。"}), 400
    try:
...
@app.route('/api/illustration/options', methods=['POST'])
def generate_illustration_options():
    req = request.json
    try:
        scenes, styles = LogicHandler.generate_illustration_options(
            req.get('gemini_key'), req.get('final_text')
        )
        return jsonify({"status": "success", "scenes": scenes, "styles": styles})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route('/api/illustration/generate', methods=['POST'])
def generate_illustration_images():
    req = request.json
    try:
        # Replicateトークンがあれば優先的に使用、なければGeminiキーをフォールバック
        token_to_use = DATA_STORE.get('replicate_token') or req.get('replicate_token') or req.get('gemini_key')
        images = LogicHandler.generate_images(
            token_to_use, 
            req.get('scene'), 
            req.get('style'), 
            req.get('custom'), 
            req.get('final_text'),
            req.get('style_choice', 'リアリスティック')
        )
