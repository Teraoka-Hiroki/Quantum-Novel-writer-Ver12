...
interface Props {
  ...
  // 【修正】生成関数を、タイプとappendフラグを受け取る形に変更
  onGenerate: (type: 'Scene Craft' | 'Character Dynamics', append: boolean) => void;
  isLoading: boolean;
}
           ...
           if (contentType && contentType.indexOf("application/json") !== -1) {
               ...
               if (result.status === 'success' && result.settings) {
                   ...
                   updateState({
                       ...
                       replicate_token: s.replicate_token || '',
                       ...
                       illustration_scenes: s.illustration_scenes || [],
                       illustration_styles: s.illustration_styles || [],
                       selected_scene: s.selected_scene || '',
                       selected_style: s.selected_style || '',
                       illustration_custom: s.illustration_custom || '',
                       illustration_style_choice: s.illustration_style_choice || 'リアリスティック',
    ...
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
            ...
            {renderSlider("粗い描写", "細かい描写", state.params.p_desc_style, 'p_desc_style')}
            {renderSlider("客観的描写", "没入的描写", state.params.p_perspective, 'p_perspective')}
            {renderSlider("視覚情報のみ", "聴覚・触覚・嗅覚・味覚の混合表現", state.params.p_sensory, 'p_sensory')}
            {renderSlider("内面隠蔽描写", "内面吐露描写", state.params.p_thought, 'p_thought')}
            {renderSlider("弛緩描写", "緊張感のある描写", state.params.p_tension, 'p_tension')}
            {renderSlider("空想的描写", "現実的描写", state.params.p_reality, 'p_reality')}
        </div>
        <div>
            <h6 className="text-blue-400 border-b border-slate-700 pb-2 mb-4 flex items-center font-semibold">
                <Users className="w-4 h-4 mr-2" /> キャラクター造形 (Character Dynamics)
            </h6>
            {renderSlider("1人", "5人", state.params.p_char_count, 'p_char_count', 0.2)}
      ...
      <div className="mt-8 flex justify-center gap-6">
        <button 
            onClick={() => onGenerate('Scene Craft', false)} // append=false (リストをリセット)
            disabled={isLoading}
            className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-bold py-3 px-8 rounded-full shadow-lg shadow-blue-500/30 transform transition hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
        >
            {isLoading ? (
        ...
        <button 
            onClick={() => onGenerate('Character Dynamics', true)} // append=true (リストに追記)
            disabled={isLoading}
            className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 px-8 rounded-full shadow-lg shadow-purple-500/30 transform transition hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
        >
            {isLoading ? (
                <span className="animate-spin mr-2">⟳</span>
            ) : (
